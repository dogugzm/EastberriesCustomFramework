            ...
            if (m_ReferenceTransform != null)
            {
                ...
                if (fitting == FitMode.MinSize)
                    rectTransform.SetSizeWithCurrentAnchors((RectTransform.Axis)axis,
                        LayoutUtility.GetMinSize(m_ReferenceTransform, axis));
                else if (fitting == FitMode.PreferredSize)
                    rectTransform.SetSizeWithCurrentAnchors((RectTransform.Axis)axis,
                        LayoutUtility.GetPreferredSize(m_ReferenceTransform, axis));
                else
                {
                    float sizeLimit = (axis == 0 ? m_HorizontalLimit : m_VerticalLimit);
                    float allowance = (axis == 0 ? m_HorizontalAllowance : m_VerticalAllowance);
                    rectTransform.SetSizeWithCurrentAnchors((RectTransform.Axis)axis,
                        Mathf.Min(LayoutUtility.GetPreferredSize(m_ReferenceTransform, axis), sizeLimit) + allowance);
                }
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m_ReferenceTransform.objectReferenceValue == null)
                EditorGUILayout.HelpBox(
                    $"A {nameof(RectTransform)} is required for {nameof(NestedContentSizeFitter.FitMode.PreferredSize)} or {nameof(NestedContentSizeFitter.FitMode.LimitedPreferredSize)}",
                    MessageType.Warning);
